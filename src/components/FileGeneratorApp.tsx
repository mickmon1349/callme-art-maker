        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <Card className="shadow-card border-border/50 bg-card/95 backdrop-blur">
              <CardHeader>
                <CardTitle className="text-primary">店家資訊輸入</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                ...
                <Button
                  ...
                  className="w-full bg-gradient-primary hover:bg-primary-hover text-primary-foreground shadow-button font-medium"
                  size="lg"
                >
                  {isGenerating ? (
                    <>
                ...
                {generatedFiles.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {generatedFiles.map((file) => (
                      <div
                        key={file.id}
                        className="flex items-center justify-between p-3 bg-secondary/50 rounded-lg border border-border/30"
                      >
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    <div className="grid grid-cols-3 gap-4 mb-4">
                      {[1, 2, 3].map((i) => (
                        <div
                          key={i}
                          className="aspect-square bg-muted rounded-lg border-2 border-dashed border-border/50 flex items-center justify-center"
                        >
                          <FileImage className="h-8 w-8 opacity-30" />
                        </div>
                      ))}
                    </div>
                    <p className="text-sm">縮圖預覽區域</p>
                  </div>
                ) : (
                    ...
                    {generatedFiles.map((file) => (
                      ...
                      >
                        <img
                          src={file.thumbnail}
                          alt={file.name}
                          className="w-full aspect-square object-cover rounded-lg border border-border/30 group-hover:shadow-lg transition-all duration-300"
                        />
